...
class PiiMaskingShareProcessor extends ShareProcessor {
  constructor(port, options) {
    ...
    port.onmessage = (event) => {
      ...
      if (command === "update-blur-options") {
        console.log("Received update-blur-options:", {
          blurRegionNorm,
          blurRadius,
        });
  ...
  onInit() {
    ...
    console.log("PII processor onInit called", canvas);
  }
  ...
  onUninit() {
    ...
    if (this.gl && !this.gl.isContextLost()) {
      ...
      if (this.progH) {
        gl.deleteProgram(this.progH);
        this.progH = null;
      }
      if (this.progV) {
        gl.deleteProgram(this.progV);
        this.progV = null;
      }
      if (this.progC) {
        gl.deleteProgram(this.progC);
        this.progC = null;
      }
